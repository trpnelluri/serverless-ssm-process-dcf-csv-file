...
async function populateErrMsg (guid, recordNum, errDetails) {
    try{
        console.error(`${clsName},${guid},populateErrMsg,errDetails: ${errDetails} recordNum: ${recordNum}`)
        let errDetailsArray = errDetails.split('^')
        let errID = errDetailsArray[0]
        let errMsgInfo = errDetailsArray[2]
        if (errMsgInfo.indexOf('{0}') > 0 ) {
            errMsgInfo = errMsgInfo.replace('{0}', ': ' + recordNum)
        }
        console.error(`${clsName},${guid},populateErrMsg,errID: ${errID} errMsgInfo: ${errMsgInfo}`)
        return errID + '-' + errMsgInfo
    } catch(err) {
        console.error(`${clsName},${guid},populateErrMsg,Error: ${err}`)
        return null
        //TBD error process 
    }
}

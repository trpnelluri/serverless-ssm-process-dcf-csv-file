                ...
                for await (const eachRec of json) {
                    ...
                    if ( dateToBeFormatted !== '' && dateToBeFormatted !== null && dateToBeFormatted !== undefined ) {
                        ...
                        if (validationResult.errorResponse){
                            ...
                            let recNumber = recordNum.toString().padStart(7, '0')
                            bodyObj.rec_num = recNumber;
                            bodyObj.actn_stus_cd = eachRec['Status/Indicator']
                            bodyObj.doc_cd = eachRec['Document Code']
                            let docDescription = eachRec['Document Description'];
                            console.log(`${clsName},${guid},getJSONDataForDCFFromS3File,Initital docDescription.length: ${docDescription.length}}`);
                            if ( docDescription.indexOf('\'') > 0 || docDescription.indexOf('\’') > 0 ) {
                                docDescription = lodash.replace(docDescription, new RegExp('’|\'', 'g'), '\'\'');
                                console.log(`${clsName},${guid},getJSONDataForDCFFromS3File,replaced single quotes}`);
                            }
                            //docDescription = docDescription.replace(/[&lf&crlf�]/gi, '');
                            //docDescription = docDescription.replace(/(\r\n|\n|\r)/gm, '');
                            //https://eslint.org/docs/latest/rules/no-control-regex
                            docDescription = lodash.replace(docDescription, new RegExp('\\r\\n,\\n,\\r,\\t', 'g'), '');
                            console.log(`${clsName},${guid},getJSONDataForDCFFromS3File, final docDescription.length: ${docDescription.length}}`);
                            bodyObj.doc_cd_desc = docDescription;
                            bodyObj.actn_dt = eachRec['Action Date'];
                            bodyObj.glbl_uniq_id = guid
                            bodyObj.fil_name = fileName
                            dcfBodyData.push(bodyObj)
                        }
                        //recordNum = recordNum + 1
                    } else { //ESMA-3714 (in case of Action Date is null/blank)
                        const actDateErrData = process.env.ACT_DATE_ERR_DATA
                        let errDetailsArray = actDateErrData.split('^')

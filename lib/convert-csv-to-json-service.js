...
class ConvertCSVToJSONService {
    ...
    async getJSONDataForDCFFromS3File(guid, fileName, s3Stream, db) {
        ...
        try {
            ...
            if(json.length === 0){
                ...
                if ( isErrorFile ) {
                    ...
                    const insertQueryRes = await BuildInsertQueryService.getInstance().buildErrMsgInsertQuery(guid, tableName, columns, additionalCols, errArray, db);

                    if ( insertQueryRes ) {

                        console.log(`${clsName},${guid},getJSONDataForDCFFromS3File,insertQueryRes: ${JSON.stringify(insertQueryRes)}`);
                        ...
                        console.log(`${clsName},${guid},getJSONDataForDCFFromS3File,sendEventRes: ${sendEventRes}`)
                        ...
                        console.log(`${clsName},${guid},getJSONDataForDCFFromS3File,sendAuditEventRes: ${sendAuditEventRes}`)
                        return SUCCESS
                    } else {
                    ...
                    if (sendAuditEventRes) {
                        console.log(`${clsName},${guid},getJSONDataForDCFFromS3File,sendAuditEventRes: ${sendAuditEventRes}`)
                        return sendAuditEventRes
                    }
